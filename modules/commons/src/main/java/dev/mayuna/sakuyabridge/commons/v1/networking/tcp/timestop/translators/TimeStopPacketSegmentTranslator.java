package dev.mayuna.sakuyabridge.commons.v1.networking.tcp.timestop.translators;

import com.esotericsoftware.kryonet.FrameworkMessage;
import dev.mayuna.sakuyabridge.commons.v1.networking.tcp.base.translator.TimeStopTranslator;
import dev.mayuna.sakuyabridge.commons.v1.networking.tcp.timestop.TimeStopPacket;
import dev.mayuna.sakuyabridge.commons.v1.networking.tcp.timestop.TimeStopPacketSegment;
        ...
        switch (context.getWay()) {
            case OUTBOUND -> {
                if (object instanceof TimeStopPacket timeStopPacket) {
                    return createSegments(context, timeStopPacket);
                }
            }
            case INBOUND -> {
                if (object instanceof TimeStopPacketSegment segment) {
                    context.setReset(true);
                    return receiveSegment(segment);
                }
            }
        }
